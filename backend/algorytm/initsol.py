    ...
    while len(ro) != len(cl_serv_k):
        ...
        while current_capacity != max_capacity:
            ...
            for item in list(cl_serv.keys()):
                if (odl[item,r[-1]] <= cl_r) and (odl[item,r[-1]] != 0) and ( item not in bkey) and item not in ro and  current_time + odl[item,r[-1]] <=cl_serv[item]["end"] and current_capacity+cl_serv[item]["demand"]<=max_capacity:
                    cl_r1 = odl[item,r[-1]] 
            ...
            if  (bkey != r[-1]) and bkey != "" and crcap<=max_capacity and crtime <=cl_serv[bkey]["end"] :
                ...
                if current_time+odl[bkey,r[-1]] <= cl_serv[bkey]["start"]:
                    ...
                    current_time= current_time+odl[bkey,r[-1]]
            else:
                break

        if len(r)==1 and len(ro) != len(cl_serv_k): raise TWerror(list(set(cl_serv_k).difference(ro)))
        rr.append(r)
        rc = copy.deepcopy(rr)

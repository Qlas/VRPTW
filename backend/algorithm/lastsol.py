...
def secalgo(initialsolution, init_val, kadencja, cl_serv,maxint, cost, odl , max_capacity):
    ...
    if len(cl_serv) ==  2 : return initialsolution,init_val
    ...
    for liczba in range (maxint):
        ...
        if len(cl_serv_knd)==1 : klient = cl_serv_knd[0]
            ...
            if item != klient and [klient,item] not in tabulist: #spradzić okna czaswe i max pojemność
                ...
                for t in trucks:
                    for i in range(1,len(t)):
                        if stime+odl[(t[i-1],t[i])]  >=cl_serv[t[i]]["start"] and stime+odl[(t[i-1],t[i])]  <= cl_serv[t[i]]["end"] and sdemand + cl_serv[t[i]]["demand"] <= max_capacity :
                            stime+=odl[(t[i-1],t[i])]
                            sdemand += cl_serv[t[i]]["demand"]
                        elif stime+odl[(t[i-1],t[i])]  <=cl_serv[t[i]]["start"] and sdemand + cl_serv[t[i]]["demand"] <= max_capacity: 
                            stime= odl[(t[i-1],t[i])]
                            sdemand += cl_serv[t[i]]["demand"]
                        else :
                            neig[klient]= b 
